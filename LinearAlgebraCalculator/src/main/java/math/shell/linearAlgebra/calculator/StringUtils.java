...
public class StringUtils
{
    ...
    public static String lastOutputInclusion(String string)
    {
        ...
        else if (string.startsWith("*") && CalculatorParser.currentType == CalculatorParser.OutputType.MATRIX)
        {
            return string.replace("*", LastEquationInput.Singleton.getLastOutput() + "*");
        }

        else if (string.startsWith("*") && CalculatorParser.currentType == CalculatorParser.OutputType.BIGDECIMAL)
        {
            return string.replace("*", LastEquationInput.Singleton.getLastOutput() + "*");
        }
    ...
    private static BigDecimal[] arrayFromString(String string)
    {
        ...
        while (true)
        {
            ...
            {
                array[nextValueToStore] = CalculatorParser.bigDecimalFromString(string, nextSpotInRow);
                nextValueToStore++;
                nextSpotInRow = nextSpotInRow + CalculatorParser.bigDecimalLengthFromString(string, nextSpotInRow);
            }
